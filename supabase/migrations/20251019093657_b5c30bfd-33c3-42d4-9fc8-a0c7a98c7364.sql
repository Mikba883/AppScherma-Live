-- Update decide_bout function to automatically delete 'info' notifications when matches are decided
...
BEGIN
  ...
  IF _bout_record.tournament_id IS NOT NULL THEN
    ...
    IF _decision = 'approve' THEN
      ...
      IF _bout_record.approved_by_a IS NOT NULL AND _bout_record.approved_by_b IS NOT NULL THEN
        ...
        -- Delete ALL 'info' notifications related to this match
      ...
      ELSE
        -- Delete previous 'info' notifications to avoid duplicates
        DELETE FROM public.notifications
        WHERE related_bout_id = _bout_id
          AND type = 'info'
          AND athlete_id = CASE WHEN _is_athlete_a THEN _bout_record.athlete_b ELSE _bout_record.athlete_a END;
  ...
  ELSE
    ...
    IF _decision = 'approve' THEN
      ...
      -- Delete 'info' notifications related to this match
      DELETE FROM public.notifications
      WHERE related_bout_id = _bout_id
        AND type = 'info';
