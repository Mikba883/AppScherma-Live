...
USING (
  athlete_id IN (
    ...
    WHERE team_id = get_current_user_team_id()
  )
);
...
AS $$
  WITH team_rankings AS (
    ...
    WHERE p.team_id = get_current_user_team_id()
  ...
  ranked AS (
    SELECT *,
           ROW_NUMBER() OVER (ORDER BY elo_rating DESC, matches_played DESC) as position
    FROM team_rankings
  )
  SELECT 
    position::INTEGER,
...
-- Add unique constraint to activity_logs
ALTER TABLE public.activity_logs 
ADD CONSTRAINT unique_athlete_week UNIQUE (athlete_id, week_start);