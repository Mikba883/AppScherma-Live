...
-- 1. Rimuovi le colonne team_id e photo_url dalla tabella profiles
ALTER TABLE public.profiles 
DROP COLUMN IF EXISTS team_id,
DROP COLUMN IF EXISTS photo_url;

-- 2. Elimina la tabella teams (non più necessaria)
DROP TABLE IF EXISTS public.teams CASCADE;

-- 3. Rimuovi la funzione get_current_user_team_id (non più necessaria)
DROP FUNCTION IF EXISTS public.get_current_user_team_id() CASCADE;

-- 4. Aggiorna la funzione summary_by_athlete per rimuovere i riferimenti a team_id
...
-- 5. Aggiorna la funzione list_bouts per rimuovere i riferimenti a team_id
...
-- 6. Aggiorna la funzione get_rankings per rimuovere i riferimenti a team_id
...
-- 7. Aggiorna register_bout_instructor per rimuovere i riferimenti a team_id
...
begin
  ...
  -- Verify both athletes exist
  if not exists (
    select 1 from public.profiles 
    where user_id = _athlete_a
  ) then
...
-- 8. Aggiorna register_bout per rimuovere i riferimenti a team_id
...
-- 9. Aggiorna handle_new_user per rimuovere i riferimenti a team_id
...
-- 10. Rimuovi la colonna team_id dalla tabella bouts
ALTER TABLE public.bouts DROP COLUMN IF EXISTS team_id;

-- 11. Aggiorna le policy RLS per rimuovere i riferimenti a team_id
