-- Fix decide_bout function by removing app_role casts that cause type resolution issues
-- This resolves the 400 error when students try to approve/reject matches
...
BEGIN
  -- Disable RLS for this transaction since function already has authorization checks
  ...
  ELSE
    -- NORMAL MATCH LOGIC
    -- FIXED: Removed ::app_role casts to avoid type resolution issues with empty search_path
    IF NOT (_bout_record.athlete_b = auth.uid() 
        OR public.has_role(auth.uid(), 'istruttore')
        OR public.has_role(auth.uid(), 'capo_palestra')) THEN
      RAISE EXCEPTION 'Non sei autorizzato a decidere questo match';
    END IF;
