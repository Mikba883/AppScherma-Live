-- Completely rebuild instructor bout registration system
...
DROP FUNCTION IF EXISTS public.trigger_update_rankings();
DROP FUNCTION IF EXISTS public.update_rankings_after_match(uuid, uuid, integer, integer);
DROP FUNCTION IF EXISTS public.register_bout_instructor(uuid, uuid, date, text, text, integer, integer);

-- Recreate the update rankings function with correct signature
