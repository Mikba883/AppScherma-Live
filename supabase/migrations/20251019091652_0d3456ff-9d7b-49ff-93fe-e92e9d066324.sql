-- Fix 1: Rimuovere notifiche "info" da register_tournament_matches
-- Fix 2: Escludere esplicitamente tornei chiusi da get_my_active_tournament

-- 1. Ricreare register_tournament_matches SENZA notifiche "info"
...
BEGIN
  ...
  -- RIMOSSO: Non creiamo più notifiche "info" generiche
  -- Le notifiche verranno create solo quando gli atleti approvano/rifiutano i match

  RETURN _tournament_id;
END;
$function$;

-- 2. Fix get_my_active_tournament per escludere tornei chiusi
...
BEGIN
  ...
  WHERE t.status = 'in_progress'
    AND t.status NOT IN ('completed', 'cancelled')  -- Esclude esplicitamente tornei chiusi
