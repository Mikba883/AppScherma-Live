-- Drop older versions of summary_by_athlete function to eliminate conflicts
DROP FUNCTION IF EXISTS public.summary_by_athlete(_from date, _to date, _gender text, _min_age integer, _max_age integer, _weapon text, _athletes uuid[]);
DROP FUNCTION IF EXISTS public.summary_by_athlete(_from date, _to date, _gender text, _min_age integer, _max_age integer, _weapon text, _athletes uuid[], _turno text);

-- Keep only the most complete version with consistent ranking logic
