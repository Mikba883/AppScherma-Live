...
interface TournamentSummaryTableProps {
  filters: Filters;
}

export const TournamentSummaryTable = ({ filters }: TournamentSummaryTableProps) => {
  const [data, setData] = useState<TournamentSummaryData[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchTournamentSummaryData();
  }, [filters]);

  const fetchTournamentSummaryData = async () => {
    setLoading(true);
    try {
      const { data: summaryData, error } = await supabase.rpc('tournament_summary_by_athlete', {
  ...
  const exportToCSV = () => {
    ...
    link.download = `riassunto_torneo_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">
          {data.length} atleti trovati (modalità torneo - ogni match conteggiato una sola volta)
        </p>
